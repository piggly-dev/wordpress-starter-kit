...
abstract class Ajaxable extends Initiable
{
	...
	protected function parseBody ( array $fields = [] ) : array
	{
		...
		{ 	
			$this->status(422)->error([
				'code' => 1,
				'message' => \sprintf($this->__translate('Invalid fields: `%s`'), implode('`, `', $errors))
			]);
		...
		if ( !$this->debug()->isDebugging() )
		{
			if ( !\check_ajax_referer($nonce_name, $nonce_param, false) )
			{
				$this->status(401)->error([
					'code' => 1,
					'message' => $this->__translate('Invalid request')
				]);
	...
	protected function need_capability ( string $capability )
	{
		...
		{
			$this->status(403)->error([ 
				'code' => 2,
				'message' => $this->__translate('Not allowed')
			]);
	...
	{ $this->_status_code = $status; return $this; }
	...
	{
		if ( $log ) { $this->debug()->force()->error($err['message'], ['code'=>$err['code']]); }
