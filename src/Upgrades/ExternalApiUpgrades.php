...
namespace Piggly\Wordpress\Upgrades;

use Piggly\Wordpress\Connector;
use Piggly\Wordpress\Helpers\WP;
use stdClass;
...
class ExternalApiUpgrades
{
	...
	public function update($transient)
	{
		...
		if (\version_compare(Connector::plugin()->version(), $remote_version, '<')
				&& \version_compare($remote_requires, \get_bloginfo('version'), '<=')
				&& \version_compare($remote_requires_php, \PHP_VERSION, '<=')) {
			$res = new stdClass();
			...
			$links_array[] = \sprintf(
				...
				\add_query_arg(
					[
						...
						'plugin' => Connector::plugin()->dirname(),
						'TB_iframe' => true,
						'width' => 772,
						'height' => 788
					],
	...
	protected function request()
	{
		...
		if ($remote === false || $this->_cache_allowed === false) {
			...
			$remote = \wp_remote_get(
				\str_replace('[slug]', Connector::plugin()->dirname(), $this->_api_url),
				[
					'timeout' => 10,
					'headers' => \array_merge([
